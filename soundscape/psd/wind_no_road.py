...
seismic = load_stations_for_distance_calculation(fullstations=True)
seismic['Station'] = pd.to_numeric(seismic['Station'], errors='coerce')
mask = (
    (seismic['Station'] >= 1500) 
)
selected = seismic.loc[mask]
stations = selected
